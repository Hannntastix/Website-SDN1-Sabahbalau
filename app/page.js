...
const Home = (schoolInfo) => {
  ...
  useEffect(() => {
    const fetchDashboards = async () => {
      try {
        const res = await fetch('https://website-sdn-1-sabahbalau.vercel.app/api/dashboard', {
          cache: 'no-store',
        });
  ...
  return (
    <div>
      {dashboards.map((schoolInfo) => (
        <div className="bg-slate-50 min-h-screen" key={schoolInfo._id}>
            ...
            <div className="max-w-7xl mx-auto px-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div>
                  ...
                  <p>{schoolInfo.contact.city}</p>
                  <p>{schoolInfo.contact.phone}</p>
                  <p>{schoolInfo.contact.email}</p>
                </div>
                <div>
                  <h3 className="text-xl font-semibold mb-4">Jam Operasional</h3>
                  <p>{schoolInfo.operationalHours.weekdays}</p>
                  <p>{schoolInfo.operationalHours.saturday}</p>
                  <p>{schoolInfo.operationalHours.sunday}</p>
                </div>
              </div>
